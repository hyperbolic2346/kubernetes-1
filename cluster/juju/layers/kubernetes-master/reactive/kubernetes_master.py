...
@when_any('kubernetes-master.components.started',
          ...
          'config.keystone-policy.changed',
          'keystone.available.auth')
@when('leadership.is_leader')
    ...
    if ca_exists and client_pass:
        ...
        if ks:
            ...
            with open(keystone_path, "w") as f:
                f.write('OS_AUTH_URL="https://{}:{}/v3"'.format(
                    ks.get('keystone_host'), ks.get('service_port')))
...
def configure_apiserver(etcd_connection_string):
    ...
    if ks and ks_ip:
        ...
        context = {}
        context['keystone-service-cluster-ip'] = ks_ip
